...
#
# Tunables (override via env vars passed to `docker run -e ...`):
#   CRUCIBLE_VLM_MODEL         (default Qwen/Qwen3-VL-32B-Instruct)
#   CRUCIBLE_VLM_SERVED_NAME   (default crucible-vlm — what clients pass as `model`)
#   VLLM_PORT                  (default 8001)
#   VLLM_MAX_LEN               (default 65536)
